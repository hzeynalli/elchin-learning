...
- Fonts: Press Start 2P (headings, buttons, HUD) + Nunito (reading text). Minecraft-inspired "Overworld" skin — palette,
  block textures and words in design/design-language.md. The old coach loop is behind `FEATURES.coach` (off since 9 Sep); Nala the buddy (`FEATURES.buddy`, `worker/src/buddy.js`) and voice are on since 10 Sep.
