...
(() => {
  ...
  function pExports(d) {
    return `<div class="grid"><section class="panel"><h2>Exports</h2><p class="muted small">The weekly PDF is a one-page report for the class teacher or tutor: topics by status, minutes, what was secured, next steps. CSVs are your backup.</p>
      ...
      <section class="panel"><h2>Question bank</h2><p class="muted small">Reading, Language Usage and Science questions come from a verified bank. Seed loads the hand-written starter content. Refill asks Claude to write a small batch of new questions for the topics with the lowest stock (a few cents per press); the Worker also refills on its own every 15 minutes.</p>
  ...
  async function act(name, ds) {
    try {
      ...
      else if (name === 'refill') { state.busy = true; render(); const r = await api('/admin/refill-bank', { max_calls: 3 }); state.busy = false; state.notice = `Refill: ${r.generated} items in ${r.calls} calls${r.stopped ? ` (stopped: ${r.stopped})` : ''}${r.skipped?.length ? `; skipped ${r.skipped.length}` : ''}.`; state.bank = null; render(); }
